...
from utils import get_last_n_tweets, to_snake_case, twitter_pics, write_df
...
async def table():
    ...
    previous = (
        datetime.datetime.strptime(current, "%Y-%m-%d-%H") - datetime.timedelta(days=1)
    ).strftime("%Y-%m-%d-%H")
    ...
    df = pd.merge(
        df_current, df_previous, on="issuer", how="outer", suffixes=("", "_previous")
    )
    ...
    df["percentage_change"] = round(
        abs((df["pricexrp"] - df["pricexrp_previous"]) / df["pricexrp_previous"] * 100),
        2,
    )
    df["direction_of_change"] = (
        np.sign(df["pricexrp"] - df["pricexrp_previous"]).fillna(0.0).astype(int)
    )
    df["price_xrp"] = df[["value", "percentage_change", "direction_of_change"]].apply(
        lambda x: x.to_json(), axis=1
    )
    ...
    df["direction_of_change"] = (
        np.sign(df["market_cap"] - df["market_cap_previous"]).fillna(0.0).astype(int)
    )
    ...
    tweets_previous = pd.DataFrame(
        tweets_list,
        columns=["twitter", "tweets_previous", "retweets_previous", "likes_previous"],
    )
    ...
    df["social_activity"] = df[["value", "percentage", "direction_of_change"]].apply(
        lambda x: x.to_json(), axis=1
    )
    output = df[
        [
    ...
    await AsyncS3FileWriter(Config.DATA_DUMP_BUCKET).write_buffer(
        "xls20/latest/NFT_Collections_Table.json", new_b
    )
